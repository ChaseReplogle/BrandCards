<?php
	...
	elseif ($current_user->membership_level = 4 )
		$confirmation_message = "<h1>" . sprintf(__('Thank you for creating a BrandCards account. Your account is now active.', 'pmpro'), get_bloginfo("name"), $current_user->membership_level->name) . "</h1>";
	else
		$confirmation_message = "<h1>" . sprintf(__('Thank you for purchaing a BrandCards plan. Your plan is now active.', 'pmpro'), get_bloginfo("name"), $current_user->membership_level->name) . "</h1>";
...
<?php if(!empty($pmpro_invoice)) { ?>
	...
	<br />
	<a href="/dashboard" class="button button-block button-primary">Let's Get Started: The Dashboard</a>
	<br />
